...
- Added support to retry failed items
    - Default retry value for all types is 0 (maintaining the current behavior).
...
- Added support for custom `CurlOptions` for any Package or Script downloads
    ...
    - Write the options you wish to pass to `curl` exactly as you would on a zsh command line.
...
These features require swiftDialog 3.0, which is still in testing, and thus features may change. swiftDialog 3.0 also requires macOS 14+.
...
- Added the ability to use custom Status icons for all Dialog List View item types.
    ...
    - There is also a new default icon option for Pending `WaitFor` items:
        - `PendingWaitForIcon`
        - All WaitFor items will get this status applied at the start of the Dialog List View, restoring pre-2.3 behavior and fulfilling #116.
...
## Improvements
- Cleaned up code for sending status updates to the main List View.
- `--version` argument now properly outputs the script version string and exits.
- Dialog version now printed to the log and report.
- `--configuration=/path/to/config.plist` is now a supported format for passing configuration file paths at runtime, which should be a nice change for Jamf admins using parameters #96 .
- New default icon for all windows.
    ...
    - Baseline will now use the swiftDialog built-in of `computer` by default which shows an icon in the style of the computer it's running on.
...
- The "EmptyPKG.pkg" is no longer included inside the default Baseline package, as it caused problems with some management systems getting confused by the multiple pkg receipts. It can now be found under the "TestingResources" folder.
...
- Fixed an issue with `WaitFor` items making the progress bar inaccurate.
...
- Remote hosted Packages that don't end with `.pkg` will now have the file extension applied automatically
    - Thank you @one8three
- Fixed a bug where Packages and Scripts that failed during the download process were not triggering a Failure dialog message (but were still logged as failures).

## Known Issues:
- Inspect Mode JSON can only be a local file path at this time, will likely add support for remote files and possibly raw/encoded strings.